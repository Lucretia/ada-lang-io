...
This is based on the tutorial by [pikuma](https://youtu.be/XfZ6WrV5Z7Y?si=oohXGQNKL72UHZFD).

You can check your process manager (or htop on \*nix) to see how much processor
time the game is using.

We need to bring in the `Timers` package:
...
We are going to cap the game to 30 FPS to start with, these are floating point
numbers, but the SDL `Milliseconds_Long` is an unsigned, so there will need to
a number of type conversions, in fact with the wrong type conversions the later
will calculations will cause an exception.

```ada
   Font              : TTFs.Fonts;
// highlight-start
   Target_FPS        : constant := 30.0;
...
The following code calculates the `Time_To_Wait` value if the game is running
too fast and then delays if it is.

Then it calculates the `Delta_Time_MS` value by which all animations can be
scaled by. Any animations scaled by this value move in _pixels per second_.
...
Now compare how much processor time the game is now using compared to what it
was doing before.

### Calculating the FPS

_N.B:_ This section doesn't work properly.
